...
from categorias.models import Item, Categoria
...
class UserProfile(models.Model):
    ...
    def items_visibles(self, categoria):
        ...
        if bool(item_list):
        	...
        	for item in item_list:
        		# Es necesario validar si la categoria a consultar es mayor, menor o igual que el item del cual es responsable
        		# La categoria a consultar es del mismo nivel que el item del cual soy responsable -> no buscar
    ...
    def validar_visibles(self, item_validar):
        """
        Funcion que devuelve si un item debiese ser visible por un usuario o no
        Parametros: item
        ...
        if bool(item_list):
            ...
            for item in item_list:
                # Es necesario validar si la categoria a consultar es mayor, menor o igual que el item del cual es responsable
                # La categoria a consultar es del mismo nivel que el item del cual soy responsable -> no buscar
                if item.categoria.get_nivel() == item_validar.categoria.get_nivel():
                    if item == item_validar:
                        visible = True
                # La categoria a consultar es menor a la categoria del item del cual es responsable -> buscar hijos
                elif item.categoria.get_nivel() < item_validar.categoria.get_nivel():
                    for hijo in item.get_hijos_controlado(item_validar.categoria.get_nivel()):
                        if hijo == item_validar:
                            visible = True
                # La categoria a consultar es mayor a la categoria del item del cual es responsable -> buscar padres
                else:
                    for padre in item.get_padres():
                        if padre == item_validar:
                            visible = True
        return visible
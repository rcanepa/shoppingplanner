...
function busquedaPlanificacionTVComp(data){
    if( data.ventas != null ){
        $("#item_planificacion_tv_comp").text("Item: " + data.itemplan.nombre + " | " + numeral(data.itemplan.precio).format('0,0'));
...
function crearTablaPlanificacionTV(data){
        ...
        $('#tab_item_planificacion_tv').scrollTop($('#tab_item_planificacion_tv')[0].scrollHeight); */
    }
    ...
    function crearTablaPlanificacionTVComp(data){
        ...
        /* El scrollbar debe aparecer posicionado por defecto al final (mostrando el año de planificacion) 
        $('#tab_item_planificacion_tv_comp').scrollTop($('#tab_item_planificacion_tv_comp')[0].scrollHeight); */
    }
    ...
    // Si se planifico el costo unitario, se entra al ciclo if
        ...
        $.each(planificacion.ventas[temporada], function(periodo_venta, venta) {
            if((venta.anio + "" + venta.periodo) == periodo.trim()){
                ...
                venta.tipo = 3;
                // Se lee el valor de la celda y se quita su formato
                venta.costo_u = numeral().unformat(obj_trabajo.getCeldaEditada().text());
